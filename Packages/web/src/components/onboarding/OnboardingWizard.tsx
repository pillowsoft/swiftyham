      ...
      <DialogContent className="max-w-md" onPointerDownOutside={(e) => e.preventDefault()}>
        ...
        {step === 'welcome' && (
          <div className="text-center py-6 px-4">
            <Radio size={48} style={{ color: 'var(--accent)' }} className="mx-auto mb-4" />
        ...
        {step === 'done' && (
          <div className="text-center py-6 px-4">
            ...
            <div className="space-y-1 text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>
              {callsign && <p>Callsign: <span className="font-mono font-bold" style={{ color: 'var(--accent)' }}>{callsign.toUpperCase()}</span></p>}
        ...
        {step !== 'welcome' && step !== 'done' && (
          <DialogFooter>
            ...
            <Button variant="ghost" size="sm" onClick={finish} className="text-xs" style={{ color: 'var(--text-muted)' }}>
              Skip
            </Button>
            <Button onClick={next}>
...
function Label({ children }: { children: React.ReactNode }) {
  return <label className="text-[10px] uppercase tracking-wider mb-1 block" style={{ color: 'var(--text-muted)' }}>{children}</label>;
}
