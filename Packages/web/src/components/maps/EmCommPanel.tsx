        ...
        <Tabs defaultValue="forms">
          ...
          <TabsContent value="forms">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs text-[var(--text-secondary)]">Create and manage ICS forms for emergency operations.</p>
              ...
              {FORMS.map(form => (
                <Card key={form.id} className="flex items-center gap-3 px-3 py-2 cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-[var(--text-muted)]">From: {form.from} → To: {form.to}</div>
                  </div>
                  <div className="text-right shrink-0">
                    <Badge variant={form.status === 'sent' ? 'green' : form.status === 'complete' ? 'gray' : 'yellow'}>{form.status}</Badge>
                    <div className="text-[10px] font-mono mt-0.5 text-[var(--text-muted)]">{form.time}</div>
                  </div>
                </Card>
              ))}
            </div>
          ...
          <TabsContent value="netlog">
            <div className="flex flex-col items-center justify-center py-12 text-[var(--text-muted)]">
              ...
              <p className="text-sm font-medium text-[var(--text-secondary)]">Net Logger</p>
          ...
          <TabsContent value="winlink">
            <div className="flex flex-col items-center justify-center py-12 text-[var(--text-muted)]">
              ...
              <p className="text-sm font-medium text-[var(--text-secondary)]">Winlink</p>
